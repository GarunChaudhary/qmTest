// app\dashboard\reports\Pass_FailEvaluationReport\page.jsx
"use client";
...
function PassFailEvaluationReport() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/reports/Pass_FailEvaluationReport", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${process.env.NEXT_PUBLIC_API_TOKEN}`,
      ...
      if (response.ok) {
        setData(result.data.passFailEvaluationData || []);
  ...
  const handleExport = async (format) => {
    try {
      ...
      const result = await response.json();
      const exportData = result.data.passFailEvaluationData || [];
  ...
  // Prepare chart data for pass/fail report
  const chartData = {
    labels: data.length > 0 ? ["Pass", "Fail"] : [],
    datasets: [
      {
        label: "Evaluations",
        data:
          data.length > 0
            ? [
                data.filter((d) => d["Pass/Fail"] === "Pass").length,
                data.filter((d) => d["Pass/Fail"] === "Fail").length,
              ]
            : [],
        backgroundColor: ["#10B981", "#EF4444"],
        borderColor: ["#0f9c6e", "#dc2626"],
        borderWidth: 1,
      },
    ],
  };
          ...
          showGraph ? (
            <div>
              <ChartReport
                chartType={chartType}
                chartData={chartData}
                title="Pass/Fail Evaluation Report"
                footerNote="Distribution of pass/fail evaluations"
                xAxisLabel="Result"
                yAxisLabel="Count"
                tooltipCallback={(context) => {
                  const value = context.dataset.data[context.dataIndex];
                  const label = context.label;
                  return [`Result: ${label}`, `Count: ${value}`];
                }}
              />
              {/* ✅ Pagination controls under chart */}
              <DynamicReportDataTable
                data={[]} // pass empty since we only need pagination UI
...
export default withAuth(PassFailEvaluationReport);
