...
function ReportsPage() {
  ...
  // Filter states - aligned with ReportFilters component
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/reports/organizationCountEvaluation", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${process.env.NEXT_PUBLIC_API_TOKEN}`,
      ...
      if (response.ok) {
        setData(result.data.OrgCountEvaluationData || []);
  ...
  const handleExport = async (format) => {
    try {
      ...
      const result = await response.json();
      const exportData = result.data.OrgCountEvaluationData || [];
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex-1 text-center">
            <h1 className="text-xl md:text-base font-semibold">
              Team/Organization wise Number of Evaluation Report
            </h1>
          </div>
          ...
          showGraph ? (
            <div>
              <ChartReport
                chartType={chartType}
                chartData={chartData}
                title="Team/Organization wise Number of Evaluation Report"
                footerNote="Team/Organization wise Number of Evaluation"
              ...
              <DynamicReportDataTable
                data={[]} // empty because we don’t need rows here
