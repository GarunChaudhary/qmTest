...
function ReportsPage() {
  ...
  const enabledFilters = ["organization", "agent"];

  // Filter state - aligned with ReportFilters component
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "/api/reports/AgentWiseEvaluationScoreReport",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        setData(result.data.AgentWiseEvaluationScore || []);
    ...
    datasets: [
      {
        ...
        borderColor: "#007fff",
        borderWidth: 1,
        agentNames: data.map((row) => row.AgentName),
        evaluatorNames: data.map((row) => row.EvaluatorName),
      },
    ],
  };
  ...
  const handleExport = async (format) => {
    try {
      ...
      const result = await response.json();
      const exportData = result.data.AgentWiseEvaluationScore || [];
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex-1 text-center">
            <h1 className="text-xl md:text-base font-semibold">
              Agent Wise Evaluation Score Report
            </h1>
          </div>
          ...
          showGraph ? (
            <div>
              <ChartReport
                chartType={chartType}
                chartData={chartData}
                title="Agent Wise Evaluation Score Report"
                footerNote="Agent Wise Evaluation Score"
                ...
                tooltipCallback={(context) => {
                  ...
                  const agentName =
                    context.dataset.agentNames[context.dataIndex];
                  const evaluatorName =
                    context.dataset.evaluatorNames[context.dataIndex];
                  return [
                    `Team/Organization: ${label}`,
                    `Agent Name: ${agentName}`,
                    `Evaluation Name: ${evaluatorName}`,
