...
function PerEvaluatorEvaluationReport() {
  ...
  const fetchPrivileges = async () => {
    try {
      ...
      if (encryptedUserData) {
        ...
        setCurrentUserFullName(user.userFullName || null);
      }
      ...
      const response = await fetch(`/api/privileges`, {
        ...
        headers: {
          ...
          moduleId: 10, 
          orgId: sessionStorage.getItem("selectedOrgId") || "", // Adjust moduleId as needed
        },
      });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "/api/reports/PerEvaluatorEvaluationReport",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        setData(result.data.PerEvaluator_EvaluationData || []);
  ...
  const chartData = {
    labels: data.map((row) => row.EvaluatorName),
    datasets: [
      {
        label: "Number of Evaluations",
        data: data.map((row) => Number(row.EvaluationsCount) || 0),
  ...
  const handleExport = async (format) => {
    try {
      ...
      const result = await response.json();
      const exportData = result.data.PerEvaluator_EvaluationData || [];
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex-1 text-center">
            <h1 className="text-xl md:text-base font-semibold">
              Per Evaluator Number of Evaluation Report
            </h1>
          </div>
          ...
          showGraph ? (
            <div>
              <ChartReport
                chartType={chartType}
                chartData={chartData}
                title="Per Evaluator Number of Evaluation Report"
                footerNote="Number of evaluations done by each evaluator"
                xAxisLabel="Evaluator Name"
                yAxisLabel="Number of Evaluations"
                tooltipCallback={(context) => {
                  ...
                  return [
                    `Evaluator: ${label}`,
                    `Form Name: ${formName}`,
                    `Evaluations Count: ${value}`,
                  ];
                }}
              />
              {/* ✅ Add pagination under chart */}
...
export default withAuth(PerEvaluatorEvaluationReport);
