// app/dashboard/reports/AgentWiseAvgEvaluationScoreReport/page.jsx
"use client";
...
function AgentWiseAvgEvaluationScoreReport() {
    ...
    try {
      ...
      const response = await fetch(`/api/privileges`, {
        ...
        headers: {
          ...
          moduleId: 10, 
          orgId: sessionStorage.getItem("selectedOrgId") || "", // Update with correct module ID for this report
        },
      });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "/api/reports/AgentWiseAvgEvaluationScoreReport",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        setData(result.data.AgentWiseAvgEvaluationScore || []);
  ...
  const chartData = {
    labels: data.map((row) => row["Agent Name"]),
    datasets: [
      {
        label: "Average Evaluation Score",
        data: data.map((row) => row["Avg Evaluation Score"]),
        backgroundColor: generateBlueShades(data.length),
        borderColor: "#007fff",
        borderWidth: 1,
        formNames: data.map((row) => row["Form Name"]),
      },
    ],
  };
  ...
  const handleExport = async (format) => {
    try {
      ...
      const result = await response.json();
      const exportData = result.data.AgentWiseAvgEvaluationScore || [];
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex-1 text-center">
            <h1 className="text-xl md:text-base font-semibold">
              Agent Wise Average Evaluation Score Report
            </h1>
          </div>
          ...
          showGraph ? (
            <div>
              <ChartReport
                chartType={chartType}
                chartData={chartData}
                title="Agent Wise Average Evaluation Score Report"
                footerNote="Agent Wise Average Evaluation Score"
                xAxisLabel="Agent"
                yAxisLabel="Score"
...
export default withAuth(AgentWiseAvgEvaluationScoreReport);
