...
function OrganizationAvgEvaluationReport() {
  ...
  const enabledFilters = ["form", "organization"];
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/reports/organizationAvgEvaluation", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${process.env.NEXT_PUBLIC_API_TOKEN}`,
      ...
      if (response.ok) {
        setData(result.data.OrgAvgEvaluationData || []);
    ...
    datasets: [
      {
        ...
        data: data.map((row) => row.AverageScore),
  ...
  const handleExport = async (format) => {
    try {
      ...
      if (encryptedUserData) {
        ...
        const user = JSON.parse(decryptedData);
        userId = user?.userId || null;
        userFullName = user?.userFullName || "Unknown";
        setCurrentUserFullName(userFullName); // optional, keeps in state
      }
      ...
      const result = await response.json();
      const exportData = result.data.OrgAvgEvaluationData || [];
      ...
      switch (format) {
        case "excel":
          exportToExcelReport(exportData, userFullName, fileName);
          break;
        case "csv":
          exportToCSVReport(exportData, userFullName, fileName);
          break;
        case "pdf":
          exportToPDFReport(exportData, userFullName, fileName);
          break;
        case "chartpdf":
          exportToPDFImageReport(exportData, userFullName, fileName);
          break;
        case "chartpng":
          exportToPNGReport(exportData, userFullName, fileName);
          break;
        default:
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex-1 text-center">
            <h1 className="text-xl md:text-base font-semibold">
              Team/Organization wise Average Evaluation Score Report
            </h1>
          </div>
          ...
          showGraph ? (
            <div>
              <ChartReport
                chartType={chartType}
                chartData={chartData}
                title="Team/Organization wise Average Evaluation Score Report"
