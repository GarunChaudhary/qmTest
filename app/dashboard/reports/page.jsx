...
const TAB_REPORT_MAP = {
  interactions: [], // embedded page, IDs not needed
  evaluation: [3, 4, 6, 5, 7, 8, 9, 10, 11],
  user: [], // embedded component
  audit: [12],
};
...
function ReportList({ reports, tabKey, router }) {
  ...
  // ⭐ Interactions tab — embed full interactions page
  if (tabKey === "interactions") {
    return <InteractionPage />;
  }

  // ⭐ User tab — embed FilteredUsersForm component
  if (tabKey === "user") {
    return <FilteredUsersForm />;
  }

  // Evaluation + Audit — show report cards
...
function ReportsCombinedPage() {
  ...
  useEffect(() => {
    ...
    if (requestedTab !== nextTab) {
      router.replace(`/dashboard/reports?tab=${nextTab}`);
    }
      ...
      <div className="space-y-5">
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          ...
          <div className="px-4 py-3 border-b border-gray-100 bg-gray-50/50">
            <div
              role="tablist"
              className="inline-flex items-center gap-0.5 p-0.5 bg-gray-200/60 rounded-lg flex-wrap"
            >
              {TAB_META.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${
                        isActive
                          ? "bg-white text-brand-primary shadow-sm border border-gray-200/70"
                          : "text-gray-500 hover:text-brand-primary hover:bg-white/40"
                      }
                    `}
                  >
                    <tab.icon
          ...
          <div role="tabpanel" className="p-5">
            {!reportsLoaded &&
            (activeTab === "evaluation" || activeTab === "audit") ? (
              <ContentSkeleton />
            ) : (
              <ReportList
                reports={reports}
