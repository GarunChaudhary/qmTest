...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* Style the calendar/clock picker icon to match theme */
  ...
  input::placeholder { color: #cbd5e1; font-size: 11.5px; }
...
const FREQUENCY_OPTIONS = [5, 10, 15, 20, 30, 45, 60, 90, 120];
...
export default function SettingsPage({ params }) {
    ...
    const fetchTimezones = async () => {
      try {
        ...
        if (response.ok && result.success && result.data.length > 0) {
          ...
          setForm((f) => ({ ...f, timezone: result.data[0].TimeZone }));
  ...
  const handleTimezoneChange = (e) => {
    ...
    setErrors((prev) => ({
      ...prev,
      timezone: selected === "" ? "This field is required" : ""
    }));
  };
  useEffect(() => {
    const fetchFrequency = async () => {
      try {
        const response = await fetch("/api/integrationWorkspace/frequencyDDL", {
          method: "GET",
          headers: {
        ...
        if (response.ok && result.success && result.data.length > 0) {
          ...
          setForm((f) => ({
            ...f,
            frequency: result.data[0].Frequencyinmintus,
          }));
        } else {
  ...
  const handleFrequencyChange = (e) => {
    ...
    setForm((f) => ({ ...f, frequency: selected }));

    setErrors((prev) => ({
      ...prev,
      frequency: selected === "" ? "This field is required" : "",
    }));
  };
  ...
  const validateForm = () => {
    const newErrors = {};
    [
      "orgId", "tokenUrl", "baseUrl", "redirectUri", "clientId", "clientSecret",
      "token", "tokenExpiresInSeconds", "refreshToken", "refreshTokenExpiresInSec",
      "timezone", "fileFormat", "destDirectory", "folderStructure",
      "startTime", "frequencyInMinutes",
    ].forEach((field) => {
      if (!form[field] || form[field].toString().trim() === "") newErrors[field] = "This field is required";
    });
    if (!form.timezone || form.timezone === "-- Select Timezone --") newErrors.timezone = "This field is required";
    setErrors(newErrors);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        router.push(`/dashboard/integrationWorkspace/Transcription/${id}`);
        setToast(true);
        setTimeout(() => setToast(false), 3000);
      } else { alert(result.message || "Failed to save configuration"); }
  ...
  const handleTest = async () => {
    try {
      ...
      const url = `https://api.wxcc-us1.cisco.com/v1/tasks?from=${from}&orgId=${form.orgId}`;
      ...
      else { alert(result?.error?.message[0].description || "Connection failed"); }
  ...
  const handleReset = () => {
    setForm({
      orgId: "", tokenUrl: "", clientId: "", clientSecret: "",
      redirectUri: "", baseUrl: "", token: "", tokenExpiresInSeconds: "",
      refreshToken: "", refreshTokenExpiresInSec: "", timezone: "",
      destDirectory: "", fileFormat: "", folderStructure: "",
      startTime: "", frequencyInMinutes: "",
      workspaceid: id,
    });
    setErrors({});
      ...
      <div className="page-wrap">
        ...
        <div className="page-header">
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            ...
            <div>
              <h1 className="page-title">Integration Configuration</h1>
              <p className="page-desc">Manage app credentials, OAuth tokens, and system preferences.</p>
            </div>
          </div>
        </div>
        ...
        <div className="card">
          ...
          {/* Section 1 — Application & Organization */}
          <div className="section">
            <div className="section-header">
              <div className="section-icon icon-blue">🏢</div>
              <div>
                <div className="section-title">Application & Organization</div>
                <div className="section-subtitle">Core app identity and fetch settings</div>
              </div>
            </div>
            <div className="grid-2">
              ...
              <div className="field">
                <label className="field-label">
                  Frequency (minutes) <span className="required">*</span>
                </label>
                ...
                <div className="input-wrap">
                  ...
                  {frequencyLoading ? (
                    ...
                    <select className="select-input" disabled>
                      <option>Failed to load frequency</option>
                    </select>
                  ) : (
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setErrors((prev) => ({
                          ...prev,
                          frequencyInMinutes: val === "" ? "This field is required" : "",
                        }));
                      }}
                    >
                      ...
                      {frequencyList.map((f) => (
                        <option key={f.Id} value={f.Frequencyinmintus}>
                          {f.Frequencyinmintus}
                        </option>
                      ))}
                    </select>
              ...
              {/* ── NEW: Start Time ── */}
              <div className="field">
                <label className="field-label">Start Time <span className="required">*</span></label>
                <div
                  className="input-wrap"
                  style={{ cursor: "pointer" }}
                  onClick={() => document.getElementById("startTimePicker").showPicker?.()}
                >
                  <span className="input-icon">📅</span>
                  <input
                    id="startTimePicker"
          ...
          <div className="section">
            <div className="section-header">
              ...
              <div>
                <div className="section-title">OAuth Endpoints</div>
                <div className="section-subtitle">Authentication URLs and redirect URIs</div>
              </div>
            </div>
            <div className="grid-2">
          ...
          {/* Section 3 — Client Credentials */}
          <div className="section">
            <div className="section-header">
              <div className="section-icon icon-amber">🛡️</div>
              <div>
                <div className="section-title">Client Credentials</div>
                <div className="section-subtitle">OAuth 2.0 client identity and secret</div>
              </div>
            </div>
            <div className="grid-2">
              <div className="field">
          ...
          {/* Section 4 — Access & Refresh Tokens */}
          <div className="section">
            <div className="section-header">
              <div className="section-icon icon-emerald">⚡</div>
              <div>
                <div className="section-title">Access & Refresh Tokens</div>
                <div className="section-subtitle">Token values and expiry durations</div>
              </div>
            </div>
            <div className="grid-2">
              <div className="field col-span-2">
          ...
          {/* Section 5 — Storage & System */}
          <div className="section">
            <div className="section-header">
              <div className="section-icon icon-blue">🗂️</div>
              <div>
                <div className="section-title">Storage & System</div>
                <div className="section-subtitle">File output, folder structure and regional settings</div>
              </div>
            </div>
            <div className="grid-2">
              <div className="field">
        ...
        <div className="footer">
          ...
          <button type="button" className="btn-secondary" onClick={handleTest} disabled={loading}>
            {loading ? <><div className="spin-loader" /> Testing…</> : "🔗 Test Connection"}
          </button>
          <button type="button" className="btn-primary" onClick={handleSave} disabled={saving}>
            {saving ? <><div className="spin-loader" style={{ borderTopColor: "white" }} /> Saving…</> : "💾 Save Configuration"}
          </button>
        </div>
